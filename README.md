...
### v1.5
- 按科目隐藏窗口：设置指定老师的课，上课自动隐藏所有窗口，下课恢复
- 提前2分钟进入上课模式（预备铃），GetCurrentEntry 自动前移 2 分钟
- 下课 2 分钟后再展开课表栏，给老师留出操作 PPT 的时间
- 删除全屏大字覆盖层弹窗，倒计时/下课提醒完全由课表栏承担
- Tab 切换动画从横向改为纵向滑动 + 0.5s 慢速过渡
- 首次打开设置页切换 Tab 不再出现重影
- 距下课 30 秒时课表栏自动展开 + 橙色高亮文字提示
- 外观+内容 两 Tab 合并为 ⏳ 倒计时 Tab（6 个 → 让设置更紧凑）
