...
> 基于 Avalonia + FluentAvalonia 构建，WinUI 3 风格界面 ｜ 当前版本：**v2.14.1**
>
...
### 实用功能
- ☁ **实时天气** — 主窗口与考试模式内置天气显示（描述/风/湿度/温度，可设刷新间隔）
...
- 🖐 **触屏友好** — 手指 / 触控笔 / 鼠标都能写（白板与屏幕批注没有滚动内容，手指直接书写）；工具栏按钮 44px 触屏热区
...
- 🖐 **触屏友好** — **手指、触控笔都能写**（v2.13.1 修：原来手指被排给滚动，触屏上写不出字）；**双指拖动 = 滚动**，鼠标滚轮/滚动条照旧；底部还有一个「手指滚动 / 手指书写」一键切换（想用单指滑屏时切过去）；缩放/翻页按钮 44px 热区
...
2. **时段模板**：定义节次时间；「适用」可选全周通用（默认）或某个星期几单独一套（如周六），点「应用」写入课表
3. **调休**：选来源星期 → 目标星期 → 应用，一键复制课表
4. **调课**：单击格子选源 → 再单击选目标 → 交换/移动/代课（双击格子可直接改课程名）
5. **课表胶囊**：主窗口内显示当天课程；上课自动收缩为进度条，距下课 1 分钟显示倒计时并展开
